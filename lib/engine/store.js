...
const INVENTORY = [
  ...
  // Frontline-ready but NOT YET PUBLISHED — the TTL detector hunts these.
  // Each has a clone match available (Studio VIN Clone is the one-tap fix).
  { vin: '5FNRL6H22NB022', model: '2023 Honda Pilot EX',     segment: 'midsize_suv',   daysInStock: 4, daysSinceFrontline: 4, photos: 0, leadsLast14: 0, status: 'frontline', cloneMatchAvailable: true },
  { vin: '2HKRW2H66NH023', model: '2023 Honda CR-V EX-L',    segment: 'compact_suv',   daysInStock: 3, daysSinceFrontline: 3, photos: 0, leadsLast14: 0, status: 'frontline', cloneMatchAvailable: true },
  { vin: '1HGCV1F88NA024', model: '2022 Honda Accord Touring',segment: 'midsize_sedan',daysInStock: 5, daysSinceFrontline: 4, photos: 0, leadsLast14: 0, status: 'frontline', cloneMatchAvailable: true },
];
...
// ─── STAGE INTAKE / THROUGHPUT (for REPLENISH-class forecasts) ──────────────
// Last-7-day rolling counts at each TTS-funnel stage. Compare to 28-day baseline
// to predict downstream starvation 3-7 days out.
const STAGE_INTAKE = {
  // TTF intake (units acquired and entering recon)
  ttf_intake_7d:      [4, 6, 5, 3, 4, 2, 3],   // recent dip in days 4-7
  ttf_intake_28d_avg: 5.2,

  // TTL intake (units becoming frontline-ready)
  ttl_intake_7d:      [3, 4, 4, 3, 3, 2, 2],
  ttl_intake_28d_avg: 3.8,

  // GEN intake (units published live)
  gen_intake_7d:      [3, 3, 3, 4, 3, 3, 2],
  gen_intake_28d_avg: 3.5,

  // APT C intake (leads received per day)
  aptc_intake_7d:     [22, 24, 19, 21, 23, 18, 14],
  aptc_intake_28d_avg: 21,

  // VISIT intake (appointments confirmed per day)
  visit_intake_7d:    [8, 9, 7, 8, 6, 7, 5],
  visit_intake_28d_avg: 8.1,

  // SELL CONV intake (visits → demos)
  sellconv_intake_7d:  [5, 6, 5, 5, 4, 5, 4],
  sellconv_intake_28d_avg: 5.4,
};
