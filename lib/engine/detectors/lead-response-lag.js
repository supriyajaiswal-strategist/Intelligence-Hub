...
function hotLeadDarkInsight(store, hotDark, slaBreaches) {
  ...
  const candidate = {
    ...
    primaryAction: {
      label: `Vini Sales Outbound · re-engage ${hotDark.length}`,
      type: ACTION_TYPE.VINI_SALES_OUTBOUND,
      payload: {
        leadIds: hotDark.map((l) => l.id),
        template: 'hot_lead_reactivation',
      },
    },
    secondaryActions: [
...
function slaBreachInsight(store, breaches) {
  ...
  const candidate = {
    ...
    primaryAction: {
      label: `Vini Sales Outbound · first touch on ${breaches.length}`,
      type: ACTION_TYPE.VINI_SALES_OUTBOUND,
      payload: { leadIds: breaches.map((l) => l.id), template: 'sla_first_touch' },
    },
    secondaryActions: [
